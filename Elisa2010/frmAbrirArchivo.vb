...
Public Class frmAbrirArchivo
   ...
   Private Sub btnObtenResultadosDA_Click(sender As System.Object, e As System.EventArgs) Handles btnObtenResultadosDA.Click
      ...
      ReDim Preserve totalCasos(cuentaNoDatos - 1)

      totalCasos(posicion).sueros = cuentaNoDatos
      totalCasos(posicion).noCaso = numcaso
      totalCasos(posicion).noSubcasos = 0
      totalCasos(posicion).subCaso = 0
      totalCasos(posicion).analisis = txtAnalisisSolicitado.Text
      totalCasos(posicion).cliente = nombreCliente
      totalCasos(posicion).texto = mensajeSobreGrafica
      totalCasos(posicion).obs = lblObservaciones.Text
      totalCasos(posicion).desdeLetra = desdex
      totalCasos(posicion).desdeValor = desdey
      totalCasos(posicion).hastaLetra = hastax
      totalCasos(posicion).hastaValor = hastay
      'Console.Write(totalCasos(posicion).sueros & " , " & totalCasos(posicion).noCaso & " , " & totalCasos(posicion).noSubcasos _
      '& " , " & totalCasos(posicion).subCaso & " , " & totalCasos(posicion).analisis & " , " & totalCasos(posicion).cliente _
      '& " , " & totalCasos(posicion).texto & " , " & totalCasos(posicion).texto & " , " & totalCasos(posicion).desdeLetra)
   ...
   Private Sub cmbNoCaso_Click(sender As Object, e As System.EventArgs) Handles cmbNoCaso.Click
      Try
         ...
         Dim oDataAdapter = New MySqlDataAdapter("SELECT  o.caso,a.id_analysis as IDAN FROM ordenes o,analisis a WHERE o.id_area=3 and o.id_status=1 and o.AnalisisSolicitados=a.id_analysis and id_analysis='" & tabla(0) & "' order by o.caso,a.id_analysis asc", oConexion)
